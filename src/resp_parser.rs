...
impl std::fmt::Display for RespType<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            RespType::RespError((error_type, error_message)) => write!(f, "Error({}, {})", error_type, error_message),
...
fn parse_one_resp_token<'a>(mut token_strings: std::collections::VecDeque<&'a str>) -> core::result::Result<(RespType, std::collections::VecDeque<&'a str>), &'static str> {
    ...
    match command {
        ...
        '-' => {
            ...
            Ok((RespType::RespError((error_type, error_message)), token_strings))
        }
        ':' => {
...
pub fn parse_resp(input_string: &str) -> std::result::Result<std::vec::Vec<RespType>, &'static str> {
